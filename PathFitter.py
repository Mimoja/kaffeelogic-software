...
def pathtosvg(path):
    ...
    for seg in path[1:]:
        ...
        last = seg
    return '<path d="' + ' '.join(segs) + '" fill="none" stroke="blue" stroke-width="1"/>\n'

def pathtosvgspots(path, size=5, colour="red", controlsSize=2, controlsColour="yellow"):
    segs = []
    for seg in path:
        try:
            x = seg.point.x
            y = seg.point.y
            controls = [((seg.point + seg.handleIn).x, (seg.point + seg.handleIn).y), ((seg.point + seg.handleOut).x, (seg.point + seg.handleOut).y)]
        except:
            x = seg[0]
            y = seg[1]
            controls = []
        segs.append('<circle cx="{}" cy="{}" r="{}" fill="{}" />\n'.format(x, y, size, colour))            
        for p in controls:
            segs.append('<circle cx="{}" cy="{}" r="{}" fill="{}" />\n'.format(p[0], p[1], controlsSize, controlsColour))            
    return ''.join(segs)

def makesvgfile(graphics, origin=(0,0), size=(100,100), pixelsize=(800,600)):
    return '<?xml version="1.0" encoding="UTF-8" ?>\n<svg width="{}" height="{}" viewBox="{} {} {} {}" xmlns="http://www.w3.org/2000/svg">\n'.format(
        pixelsize[0], pixelsize[1], origin[0], origin[1], size[0], size[1])+ \
        graphics + '</svg>'
